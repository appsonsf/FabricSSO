...
using SecurityTokenWeb.UtilityLib;
using Microsoft.Extensions.Configuration;
using SecurityTokenWeb;
...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(string returnUrl)
        {
            ...
            ViewBag.ScanLoginServiceUri = _config.GetSection("IdSvr").GetValue<string>("ScanLoginServiceUri");
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            var (user, userManager) = await this._handleLoginService.RetrieveUserAsync(model.Username);
            //remove all the blank character

            if (user == null)
            {
                await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "无效的凭证信息"));
                ModelState.AddModelError("", StsAccountStatusCode.AccountError.ToDescription());
                return View(vm);
            }
            else if (user.LockoutEnd.HasValue && user.LockoutEnd.Value > DateTimeOffset.Now)
            {
                await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "无效的凭证信息"));
                ModelState.AddModelError("", string.Format("您的账户已经锁定,自动解锁时间:{0},或请联系管理员马上解锁", user.LockoutEnd.Value.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss")));
                return View(vm);
            }
            else if (user.IsActive.HasValue && !user.IsActive.Value) //下一个版本解决这个问题
            {
                await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "无效的凭证信息"));
                ModelState.AddModelError("", string.Format("您的账户已经被禁用,如有疑问请联系管理员!"));
                return View(vm);
            }

            switch (model.LoginType)
            {
                case LoginType.UserNamePwdLogin:
                    if (!await this._handleLoginService.CheckNormalPasswordLoginAsync(user, userManager, model.Password))
                    {
                        await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "无效的凭证信息"));
                        ModelState.AddModelError("", StsAccountStatusCode.PasswordError.ToDescription());
                        return View(vm);
                    }
                    break;
                case LoginType.MobileValidateCode:
                    if (!await this._handleLoginService.CheckMobileLoginAsync(user, model.Password))
                    {
                        await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "无效的凭证信息"));
                ...
                case LoginType.TempPassword:
                    ...
                    {
                        await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "无效的凭证信息"));
                        ModelState.AddModelError("", StsAccountStatusCode.TempPasswordError.ToDescription());
                        return View(vm);
                    }
                    break;
            }
            //login success
            if (model.LoginType == LoginType.UserNamePwdLogin)
            {
        ...
        private (TestUser user, string provider, string providerUserId, IEnumerable<Claim> claims) FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            var user = default(TestUser); // _users.FindByExternalProvider(provider, providerUserId);
